...
n_sample = 50 #number of samples in space
m_sample = 3 #number of samples in time
...
###########collecting samples ################
'''
neste rascunho vou fazer o seguinte:
1º - vou tentar chegar à condição inicial com amostas no inicio o processo
1.1 - vou ajustar as amostras fixas ok
1.2 - tentar construir  o gradiente 
1.3 - usar o gradiente descendente
1.4 - estimar o erro comentido pela assimilação ok?
2º - ajustar para que as amostras sejam aleatórias
3º - tentar assimilar a condição inicial com essas amostras aleatórias.
'''
#obter as amostar de forma fixa
...
# obter as amostras de maneira aleatória
'''for _ in range(m_sample):
    #random_time = rd.uniform(sample_window[0],sample_window[1])
    random_time = np.array([x/2 for x in range(m_sample)])
    #print(random_time)
...
for j in range(10):
    ...
    for i in range(int(math.ceil(N*2.5/(2*L)))):
        ...
        plt.xlim(-1, 2.5) # x limit
        plt.plot(X_obs,Y_obs, marker = 'o', ms = 1, ls = '', color = 'red')
        plt.plot(x_values, vet, lw = 1, color = 'black', )
        plt.title(f'Execução {j+1} de 1000.  T= {i*delta_t*8:.1f} h. de 20h')
        plt.pause(0.1)
        u_foreward = np.roll(vet,1)
        u_backward = np.roll(vet,-1) 
        vet = kappa2*u_foreward+kappa1*u_backward
        if i == 1:
            break
    d = u  
    ...
    for k in range(len(X_obs)):
        for i in range(len(x_values)):
            if X_obs[k] == x_values[i]:
                Cost1 += 0.5*(abs(Y_obs[k]-u[i])**2)
                d[i] = Y_obs[k]-u[i]
    if Cost1 > Cost:
    ...
    Cost = Cost1
    #for _ in range(n_sample):#<----- parei aqui
    d_foreward = np.roll(d,1)
    d_backward = np.roll(d,-1)
    New_u = u + 0.1*(kappa1*d_foreward+kappa2*d_backward)
    u = New_u
print('End of process.')
plt.show()

print(f'No total o processo teve {total} reduções de custo')
